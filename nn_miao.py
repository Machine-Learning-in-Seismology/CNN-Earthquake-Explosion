...
from model_miao import build_nn, get_early_stop
...
# FFT
EXP_DIR = MASTER_DIR + "Miao/EXP/"
ERT_DIR = MASTER_DIR + "Miao/EQ/"
eq_spec = []
ex_spec = []
#for i in range(5):
...
# Remove arrays with NaN
ex_spec = np.array(ex_spec)
ex_errors = []
for i, array in enumerate(ex_spec):
	array_sum = np.sum(array)
	array_has_nan = np.isnan(array_sum)
	if array_has_nan == True:
		ex_errors.append(i)
ex_spec = np.delete(ex_spec, ex_errors, axis=0)

eq_spec = np.array(eq_spec)
eq_errors = []
for i, array in enumerate(eq_spec):
	array_sum = np.sum(array)
	array_has_nan = np.isnan(array_sum)
	if array_has_nan == True:
		eq_errors.append(i)
...
X_spec = np.squeeze(X_spec, axis=1)
...
for train_index, test_index in kf.split(X_spec):
	...
	for seed in range(5):
		for epoch in EPOCHS:
			...
			results.to_csv("results/miao_results" + timestr + ".csv", sep="\t", encoding='utf-8')
	fold += 1
